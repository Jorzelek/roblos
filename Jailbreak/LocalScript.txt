...
local animIds = {
	punchLeft = "rbxassetid://13260827430",
	punchRight = "rbxassetid://13260829947",
	crawl = "rbxassetid://13260825079"
}
local anims = {}
...
local function tweenCameraCf(cf, tweenTime, callback)
	local tween = tS:Create(camera, TweenInfo.new(tweenTime, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, 0, false, 0), {CFrame = cf})
	tween:Play()
...
local function tweenCameraStyle(cf, tweenTime, style, callback)
	if style == nil then style = Enum.EasingStyle.Linear end
...
do
	function cutsceneCity(boolean)
		...
		local function doo()
			...
			tweenCameraCf(part2.CFrame, tweenTime, function()
				if not cutsceneCityPlaying then return end
...
guis.ScreenGui.Team.Prisoner.MouseButton1Down:Connect(function()
	...
	setCameraCf(cameraPart.CFrame + Vector3.new(5, 1, -5))
	tweenCameraStyle(cameraPart.CFrame, tonumber(1), Enum.EasingStyle.Cubic)
end)
...
guis.ScreenGui.Team.Police.MouseButton1Down:Connect(function()
	...
	cutsceneCityPlaying = false
	setCameraCf(camerasFolder.Cop.CFrame + Vector3.new(5, 1, 5))
	tweenCameraStyle(camerasFolder.Cop.CFrame, tonumber(1), Enum.EasingStyle.Cubic)
end)
...
local function updateChassisLowQuality(packet, dt)
	...
	local wheelRotation = packet.WheelRotation + distance / (model:FindFirstChild("Preset").WheelFrontRight.Wheel.Size.Y * 0.5 * math.pi)
